...
DROP USER IF EXISTS `restadmin`@`%`; -- @ % allows all ip so request from any ip is allowed
CREATE DATABASE IF NOT EXISTS restdb CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
CREATE USER IF NOT EXISTS `restadmin`@`%` IDENTIFIED WITH caching_sha2_password BY 'admin';
